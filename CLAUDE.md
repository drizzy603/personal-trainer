...
- **Scene lifecycle.** `AppDelegate.swift` also holds `SceneDelegate` and `Info.plist` carries `UIApplicationSceneManifest` (storyboard `Main`, delegate `$(PRODUCT_MODULE_NAME).SceneDelegate`); URL opens and user activities reach Capacitor's `ApplicationDelegateProxy` and the `trovo://` deep link through it. Added in build 46 because Apple requires UIScene adoption for apps built against the iOS 26+ SDK. It was NOT the cause of the build-44/45 crash report — an A/B on an iOS 27 simulator showed the same binary launching with the manifest stripped (see the crash memory). Keep it; it is correct either way.
