...
- `screenshots/` — App Store screenshot pipeline. `make-seed.js` (demo data + store fixups → `out/store-seed.json`) then `shoot.py` (fresh sim container, sqlite seed injection, kt_dev_nav-driven capture of all ten 1320×2868 PNGs into `screenshots/out/`). Prep steps in shoot.py's header; README has the full flow. Legacy manual helpers: `launch-sim.sh`, `seed-demo.js`, `take-shot.sh`.
