...
run('week stats are honest; wrapped block renders and dismisses', async () => {
  ...
  try {
    const out = await app.page.evaluate(() => {
      ...
      // Make the prior week lighter — now this one is genuinely heaviest.
      // (A first-ever week with NO prior weeks earns nothing by default.)
      lsSet('kt_sessions', [
        { id: 1, date: todayISO(), type: 'Push', week: 2,
          exercises: [{ name: 'Bench Press', sets: 3, reps: [8, 8, 8], weight: 100, isMain: true }], prs: [] },
        { id: 2, date: lastISO, type: 'Push', week: 1,
          exercises: [{ name: 'Bench Press', sets: 1, reps: [8], weight: 50, isMain: true }], prs: [] },
      ]);
      const st2 = _weekStats();
      const earned = st2.heaviest === true && st2.volume === 2400;
      // No prior week at all → the claim is never asserted.
      lsSet('kt_sessions', [{ id: 1, date: todayISO(), type: 'Push', week: 1,
        exercises: [{ name: 'Bench Press', sets: 3, reps: [8, 8, 8], weight: 100, isMain: true }], prs: [] }]);
      const firstWeekQuiet = _weekStats().heaviest === false;
      // Restore the earned state for the render check below.
      lsSet('kt_sessions', [
        { id: 1, date: todayISO(), type: 'Push', week: 2,
          exercises: [{ name: 'Bench Press', sets: 3, reps: [8, 8, 8], weight: 100, isMain: true }], prs: [] },
        { id: 2, date: lastISO, type: 'Push', week: 1,
      ...
      return { honest, earned, firstWeekQuiet, card, retired };
    });
    ...
    assert(out.firstWeekQuiet, 'a first-ever week asserts nothing');
